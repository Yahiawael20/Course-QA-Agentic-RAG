"""
Course QA Agent — Streamlit Frontend
=====================================

A premium, dark-themed Streamlit UI for an Agentic RAG backend
(Planner -> Tool -> Retriever -> Vector Store -> LLM).

This file is ONLY the frontend. It talks to the existing backend
(agent/, tools/, sources/) through `get_agent_response()`, which
normalizes whatever the backend returns into a single response
dict the UI renders from.
...
def parse_quiz(text: str):
    """Parse quiz tool text into structured {question, options, answer} blocks."""
    blocks = re.split(r"\n(?=\s*\d+[\.\)])", text.strip())
    questions = []
    ...
    for block in blocks:
        ...
        for line in lines[1:]:
            ...
            elif re.match(r"(?i)^(correct answer|answer)\s*[:\-]", stripped):
                answer_line = re.sub(r"(?i)^(correct answer|answer)\s*[:\-]\s*", "", stripped).strip()

        if options:
            questions.append({"question": q_line, "options": options, "answer": answer_line})
...
def get_agent_response(question: str) -> dict:
    ...
    for step in steps:
        ...
        if tool_name == "quiz" and quiz_data is None:
            quiz_data = parse_quiz(answer_text)
    ...
    return {
        "answer": "\n\n".join(answer_chunks) if answer_chunks else "No answer was produced.",
...
def render_planner_decision(planner):
    if not planner:
        return
    tool_label = TOOL_DISPLAY_NAMES.get(planner.get("selected_tool", ""), str(planner.get("selected_tool", "")).title())
    ...
    st.markdown(
        ...
        <div class="planner-row">
            <div class="planner-item">
                <div class="section-label" style="margin-bottom:0;">Selected Tool</div>
                <div class="planner-value">{tool_label}</div>
            </div>
            <div class="planner-item">
...
def render_quiz(quiz_items):
    ...
    for i, item in enumerate(quiz_items, start=1):
        options_html = "".join(f'<div class="quiz-option">{opt}</div>' for opt in item["options"])
        answer_html = f'<div class="quiz-answer">✅ {item["answer"]}</div>' if item.get("answer") else ""

        st.markdown(
            f"""
            <div class="quiz-card">
                <div class="quiz-q-num">Question {i}</div>
                <div style="font-weight:600; margin-bottom:0.5rem;">{item["question"]}</div>
                {options_html}
                {answer_html}
            </div>
            """,
...
def render_answer(answer_text, tool_used, comparison_table, quiz_items):
    st.markdown('<div class="glass-card">', unsafe_allow_html=True)
    st.markdown('<div class="card-title">🤖 Answer</div>', unsafe_allow_html=True)
    st.markdown(answer_text)
    st.markdown('</div>', unsafe_allow_html=True)

    if tool_used == "compare" and comparison_table:
        render_comparison_table(comparison_table)

    if tool_used == "quiz" and quiz_items:
        render_quiz(quiz_items)
...
if response:
    render_answer(
        ...
        tool_used=response["planner"]["selected_tool"],
        comparison_table=response["comparison_table"],
        quiz_items=response["quiz"],
    )
